...
export const TicketStore = {
   ...
   */
  async save(ticketId, analyticsKey) {
    const AS = getAsyncStorage();
    if (!AS) {
      console.warn('[TicketStore] @react-native-async-storage/async-storage is not installed — ' + 'ticket will not persist across app restarts. ' + 'Run: npx expo install @react-native-async-storage/async-storage');
      return;
    }
    try {
      await AS.setItem(STORAGE_KEY, JSON.stringify({
        ticketId,
        analyticsKey
      }));
