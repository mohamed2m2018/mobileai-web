...
export function createProvider(provider = 'gemini', apiKey, model, proxyUrl, proxyHeaders, enableWebSearch) {
  switch (provider) {
    case 'openai':
      return new OpenAIProvider(apiKey, model || 'gpt-4.1-mini', proxyUrl, proxyHeaders);
    case 'gemini':
    default:
      return new GeminiProvider(apiKey, model || 'gemini-3.1-flash-lite', proxyUrl, proxyHeaders, enableWebSearch);
  }
}
