...
const VIDEO_TYPES = new Set(['Video', 'ExpoVideo', 'RCTVideo', 'VideoPlayer', 'VideoView']);
 ...
 */
function getFiberRootFromDevTools() {
  try {
...
export function walkFiberTree(rootRef, config) {
  ...
  function processNode(node, depth = 0, isInsideInteractive = false, ancestorInteractionSignature = null, currentZoneId = undefined) {
    ...
    if (componentName === 'AIZone' && props.id) {
      ...
      const blockSummaries = Array.isArray(props.blocks) ? props.blocks.map(block => {
        if (!block?.name) return null;
        const propNames = block.propSchema ? Object.keys(block.propSchema) : [];
        return propNames.length > 0 ? `${block.name}(${propNames.join(',')})` : block.name;
      }).filter(summary => typeof summary === 'string' && summary.trim().length > 0) : Array.isArray(props.templates) ? props.templates.map(template => template?.displayName || template?.name).filter(name => typeof name === 'string' && name.trim().length > 0) : [];
      const permStr = permissions.length ? ` | permissions: ${permissions.join(', ')}` : '';
      const blockStr = blockSummaries.length ? ` | blocks: ${blockSummaries.join(', ')}` : '';
    ...
    if (isImageNode) {
      ...
      const src = typeof props.source === 'object' && props.source?.uri ? props.source.uri : '';
      const attrs = [context ? `in="${context}"` : '', alt ? `alt="${alt}"` : '', src ? `src="${src}"` : ''].filter(Boolean).join(' ');
    ...
    if (isVideoNode) {
      ...
      const attrs = [context ? `in="${context}"` : '', paused !== null ? `state="${paused ? 'paused' : 'playing'}"` : '', src ? `src="${src}"` : '', poster ? `poster="${poster}"` : ''].filter(Boolean).join(' ');
...
const SCROLLABLE_TYPES = new Set(['ScrollView', 'RCTScrollView', 'FlatList', 'SectionList', 'VirtualizedList']);
 ...
 */
const PAGER_TYPES = new Set(['RNCViewPager', 'PagerView', 'ViewPager', 'RNViewPager', 'TabView', 'MaterialTabView', 'ScrollableTabView', 'RNTabView']);
/**
 ...
 */
function findScreenFiberNode(rootFiber, screenName) {
 ...
 */
export function findScrollableContainers(rootRef, screenName) {
  const fiber = getFiberFromRef(rootRef);
  if (!fiber) {
