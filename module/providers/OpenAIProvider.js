...
export class OpenAIProvider {
  constructor(apiKey, model = 'gpt-4.1-mini', proxyUrl, proxyHeaders) {
    if (proxyUrl) {
      this.baseUrl = proxyUrl.endsWith('/') ? `${proxyUrl}v1/chat/completions` : proxyUrl;
      this.headers = {
  ...
  async generateContent(systemPrompt, userMessage, tools, _history, screenshot) {
    ...
    try {
      ...
      if (tokenUsage) {
        logger.info('OpenAIProvider', `Tokens: ${tokenUsage.promptTokens} in / ${tokenUsage.completionTokens} out / $${tokenUsage.estimatedCostUSD.toFixed(6)}`);
      }
  ...
  parseAgentStepResponse(data, tools) {
    ...
    if (!toolCall?.function) {
      logger.warn('OpenAIProvider', 'No tool call in response. Text:', message?.content);
      if (looksLikeInternalPlanText(message?.content)) {
        logger.error('OpenAIProvider', `Provider returned plan-like plain text instead of agent_step. text="${message?.content}" message=${JSON.stringify(message)}`);
      } else {
        logger.warn('OpenAIProvider', `Malformed agent_step response without tool_call. message=${JSON.stringify(message)}`);
      }
      return {
        toolCalls: [{
          name: 'done',
    ...
    try {
      ...
      logger.error('OpenAIProvider', 'Failed to parse tool arguments:', toolCall.function.arguments);
      return {
        toolCalls: [{
          name: 'done',
          args: {
    ...
    if (!actionName) {
      logger.warn('OpenAIProvider', 'No action_name in agent_step. Falling back to done.');
      if (looksLikeInternalPlanText(message?.content) || looksLikeInternalPlanText(reasoning.plan)) {
        logger.error('OpenAIProvider', `agent_step missing action_name with plan-like content. text="${message?.content || ''}" plan="${reasoning.plan}" rawArgs=${JSON.stringify(args)}`);
      } else {
        logger.warn('OpenAIProvider', `agent_step missing action_name. rawArgs=${JSON.stringify(args)}`);
      }
      return {
        toolCalls: [{
          name: 'done',
    ...
    logger.info('OpenAIProvider', `Parsed: action=${actionName}, plan="${reasoning.plan}"`);
    return {
      toolCalls: [{
        name: actionName,
        args: actionArgs
