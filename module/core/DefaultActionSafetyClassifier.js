...
function inferScope(userRequest) {
...
function capabilityFromEffect(toolName, effect) {
...
function classifyLabel(label, screenText, toolName) {
...
function riskForCapability(capability) {
...
function decisionForCapability(scope, capability, unknownActionDecision) {
  ...
  if (capability === 'state.modify') {
    ...
    return {
      ...
      capability,
      risk,
      reason: shoppingPrep ? 'Routine shopping preparation state change within the approved task scope.' : 'This action may modify app state.',
  ...
  return {
    decision: LOW_RISK_CAPABILITIES.has(capability) || capability === 'ui.fill' || capability === 'ui.select' ? 'allow' : 'ask',
    confidence: 0.86,
    scope,
    capability,
    risk,
...
function buildHeuristicDecision(input, unknownActionDecision) {
  const scope = inferScope(input.userRequest);
  const effectCapability = capabilityFromEffect(input.toolName, input.toolEffect);
  const capability = effectCapability ?? classifyLabel(input.targetElement?.label ?? '', input.screenContent ?? input.screen?.elementsText ?? '', input.toolName);
  return decisionForCapability(scope, capability, unknownActionDecision);
}
function normalizeModelDecision(raw, fallback) {
...
export function resolveDefaultGuardModel(config) {
...
export class DefaultActionSafetyClassifier {
  guardProvider = null;
  constructor(options) {
    this.options = options;
  }
  async classifyScreen(input) {
    const decisions = Object.fromEntries(input.screen.elements.flatMap(element => {
  ...
  async classifyAction(input) {
    const fallback = buildHeuristicDecision(input, this.options.config.actionSafety?.unknownActionDecision ?? 'ask');
    ...
    try {
      ...
      const result = await provider.generateContent(DEFAULT_GUARD_SYSTEM_PROMPT, buildGuardUserPrompt(input, fallback), [CLASSIFY_ACTION_TOOL], []);
  ...
  getGuardProvider() {
    ...
    this.guardProvider = createProvider(config.provider ?? 'gemini', config.apiKey, resolveDefaultGuardModel(config), config.proxyUrl, config.proxyHeaders);
    return this.guardProvider;
  }
}
function buildGuardUserPrompt(input, fallback) {
...
const CLASSIFY_ACTION_TOOL = {
  ...
  parameters: {
    ...
    scope: {
      type: 'string',
      enum: ['read_or_lookup', 'support_investigation', 'form_assistance', 'shopping_preparation', 'account_management', 'communication_preparation', 'unknown_task'],
      description: 'Inferred user task scope',
      required: true
    },
    capability: {
      type: 'string',
      enum: ['screen.read', 'ui.navigate', 'ui.scroll', 'ui.fill', 'ui.select', 'ui.dismiss', 'state.modify', 'content.send', 'external.open', 'support.escalate', 'payment.commit', 'order.commit', 'account.security', 'privacy.sensitive', 'destructive', 'unknown'],
    ...
    reason: {
      type: 'string',
      description: 'Short audit reason',
      required: true
    },
    userMessage: {
      type: 'string',
    ...
    requiresFreshApproval: {
      type: 'boolean',
      description: 'Whether existing workflow approval cannot be reused',
      required: true
    }
  },
  execute: async () => 'classified'
};
//# sourceMappingURL=DefaultActionSafetyClassifier.js.map