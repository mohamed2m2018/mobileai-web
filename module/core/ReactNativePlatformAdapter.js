...
import { walkFiberTree, findScrollableContainers } from "./FiberTreeWalker.js";
import { getChild, getSibling, getProps, getStateNode, getParent } from "./FiberAdapter.js";
import { dismissAlert } from "./NativeAlertInterceptor.js";
import { globalBlockRegistry } from "./BlockRegistry.js";
import { globalZoneRegistry } from "./ZoneRegistry.js";
...
function extractPickerOptions(element) {
...
function sanitizePropValue(value) {
  ...
  if (typeof value === 'object') {
    ...
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, sanitizePropValue(item)]).filter(([, item]) => item !== undefined));
  }
  return undefined;
}
function parsePropsArg(rawProps) {
  ...
  return sanitizePropValue(parsed);
}
...
export class ReactNativePlatformAdapter {
  ...
  async captureScreenshot() {
    try {
      ...
      if (error.message?.includes('Cannot find module') || error.code === 'MODULE_NOT_FOUND' || error.message?.includes('unknown module')) {
        logger.warn('ReactNativePlatformAdapter', 'Screenshot requires react-native-view-shot. Install it in the host app with: npx expo install react-native-view-shot');
      } else {
  ...
  buildNestedParams(path, leafParams) {
  ...
  async typeText(index, text) {
    ...
    if (fiberNode) {
      const innerTextInputFiber = findFiberNode(fiberNode, node => typeof getProps(node)?.onChangeText === 'function', 15);
    ...
    if (fiberNode) {
      ...
      const nativeOnChangeFiber = fiberProps?.onChange ? fiberNode : findFiberNode(fiberNode, node => typeof getProps(node)?.onChange === 'function');
      const fiberStateNode = getStateNode(fiberNode);
      const nativeStateFiber = fiberStateNode?.setNativeProps ? fiberNode : findFiberNode(fiberNode, node => {
  ...
  async scroll(direction, amount = 'page', containerIndex = 0) {
    ...
    if (!container) {
      const available = containers.filter(entry => !entry.isPagerLike).map(entry => `[${entry.index}] ${entry.label} (${entry.componentName})`).join(', ');
    ...
    if (container.isPagerLike) {
      const scrollableAlts = containers.filter(entry => !entry.isPagerLike).map(entry => `[${entry.index}] ${entry.label} (${entry.componentName})`).join(', ');
      const suggestion = scrollableAlts ? `Try scrolling a content container instead: ${scrollableAlts}` : 'Use tap on the tab labels to switch tabs instead of scrolling.';
    ...
    const offsetBefore = typeof scrollRef.scrollToOffset === 'function' ? scrollRef._scrollMetrics?.offset ?? 0 : 0;
    try {
      if (amount === 'toEnd') {
        ...
        if (typeof scrollRef.scrollToOffset === 'function') {
          ...
          const newOffset = direction === 'down' ? currentOffset + scrollAmount : Math.max(0, currentOffset - scrollAmount);
          scrollRef.scrollToOffset({
            offset: newOffset,
            animated: true
          });
          ...
          scrollRef.scrollTo({
            y: direction === 'down' ? currentY + scrollAmount : Math.max(0, currentY - scrollAmount),
            animated: true
          });
        }
      }
      await new Promise(resolve => setTimeout(resolve, 1500));
      const offsetAfter = typeof scrollRef.scrollToOffset === 'function' ? scrollRef._scrollMetrics?.offset ?? 0 : -1;
  ...
  async selectPicker(index, value) {
    ...
    if (options.length > 0) {
      const match = options.find(option => String(option.value) === value || option.label.toLowerCase() === value.toLowerCase());
      if (!match) {
  ...
  async navigate(screen, rawParams) {
    ...
    try {
      const params = rawParams ? typeof rawParams === 'string' ? JSON.parse(rawParams) : rawParams : undefined;
...
//# sourceMappingURL=ReactNativePlatformAdapter.js.map